...
// Fireworks, a proud pirouette, a gleaming floor — then the room stays clean
// until somebody (three years old, probably) makes a brand-new mess.
const WinParty = {
  name: 'winParty',
  weight: 0,
  canRun: () => false,
  ...
  update(g, dt) {
    ...
    st.t += dt;
    r.targetSpeed = 0;
    // proud pirouette that eases out
    if (st.t < 2.8) r.spinExtra += dt * 5.5;
    else r.spinExtra *= 1 - 7 * dt;
    st.burstT -= dt;
    if (st.burstT <= 0 && st.hops < 3) {
      st.burstT = 1.05;
      st.hops++;
      r.hop(240);
      g.particles.confettiBurst(r.x, r.y - 40, 22);
    }
    // the freshly-cleaned floor gleams all over
    st.shineT -= dt;
    if (st.shineT <= 0 && st.t < 3.6) {
      st.shineT = 0.14;
      const p = g.room.randomFloorPoint(30);
      g.particles.sparkle(p.x, p.y, 2);
    }
    if (st.t > 4.2 && Math.abs(r.spinExtra) < 0.15) {
      r.spinExtra = 0;
      g.sound.happyBeeps(5);
      this.finished = true;
    }
